import{ba as t,c9 as e,ca as r,cd as o,ce as i}from"./3ptQIBJb.js";import{n as s}from"./LgAAmdpy.js";import"./D0MC3uiJ.js";import"./ByssTI6i.js";import{Q as a}from"./D7KjvUes.js";import{c as n}from"./nddBALH4.js";function c(t,e,r){if(t===e)return!1;return(t-e<0?e-t:t-e)<=r+.1}const l={generate({uri:e,size:r,logoSize:o,dotColor:i="#141414"}){const s=[],n=function(t,e){const r=Array.prototype.slice.call(a.create(t,{errorCorrectionLevel:e}).modules.data,0),o=Math.sqrt(r.length);return r.reduce((t,e,r)=>(r%o===0?t.push([e]):t[t.length-1].push(e))&&t,[])}(e,"Q"),l=r/n.length,h=[{x:0,y:0},{x:1,y:0},{x:0,y:1}];h.forEach(({x:e,y:r})=>{const o=(n.length-7)*l*e,a=(n.length-7)*l*r,c=.45;for(let n=0;n<h.length;n+=1){const e=l*(7-2*n);s.push(t`
            <rect
              ...
              height=${0===n?e-5:e}
              x= ${0===n?a+l*n+2.5:a+l*n}
              y= ${0===n?o+l*n+2.5:o+l*n}
            />
          `)}});const u=Math.floor((o+25)/l),p=n.length/2-u/2,d=n.length/2+u/2-1,f=[];n.forEach((t,e)=>{t.forEach((t,r)=>{if(n[e][r]&&!(e<7&&r<7||e>n.length-8&&r<7||e<7&&r>n.length-8||e>p&&e<d&&r>p&&r<d)){const t=e*l+l/2,o=r*l+l/2;f.push([t,o])}})});const g={};return f.forEach(([t,e])=>{g[t]?g[t]?.push(e):g[t]=[e]}),Object.entries(g).map(([t,e])=>{const r=e.filter(t=>e.every(e=>!c(t,e,l)));return[Number(t),r]}).forEach(([e,r])=>{r.forEach(r=>{s.push(t`<circle cx=${e} cy=${r} fill=${i} r=${l/2.5} />`)})}),Object.entries(g).filter(([t,e])=>e.length>1).map(([t,e])=>{const r=e.filter(t=>e.some(e=>c(t,e,l)));return[Number(t),r]}).map(([t,e])=>{e.sort((t,e)=>t<e?-1:1);const r=[];for(const o of e){const t=r.find(t=>t.some(t=>c(o,t,l)));t?t.push(o):r.push([o])}return[t,r.map(t=>[t[0],t[t.length-1]])]}).forEach(([e,r])=>{r.forEach(([r,o])=>{s.push(t`
              <line
                x1=${e}
                x2=${e}
                y1=${r}
